...
@interface ZHZSlideMenuController ()<UIGestureRecognizerDelegate>
...
@implementation ZHZSlideMenuController
...
- (void)showViewController:(UIViewController *)viewController{
    if ([self.contentViewController isKindOfClass:[UINavigationController class]]) {
        ...
        NSAssert([self.contentViewController conformsToProtocol:@protocol(ZHZContentViewControllerDelegate)], @"ContentViewController不是UINavigationController后者ContentViewController没有UINavigationController,请在ContentViewController中实现YQContentViewControllerDelegate协议");
        id<ZHZContentViewControllerDelegate> delegate = (id<ZHZContentViewControllerDelegate>)self.contentViewController;
        if ([delegate respondsToSelector:@selector(zhz_navigationController)]) {
            UINavigationController *nav = [delegate zhz_navigationController];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer{
    ...
    if (self.menuHidden){
        if (point.x <= LeftMarginGesture){
            ...
            } else if ([self.contentViewController conformsToProtocol:@protocol(ZHZContentViewControllerDelegate)]) {
                id<ZHZContentViewControllerDelegate> delegate = (id<ZHZContentViewControllerDelegate>)self.contentViewController;
                nav = [delegate zhz_navigationController];
            }
            if (nav) {
